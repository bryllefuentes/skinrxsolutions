		...
		if (!empty($GLOBALS['simba_two_factor_authentication'])) {
			...
			if ($is_2fa_plugin_active) {
				if (file_exists(__DIR__.'/simba-tfa/premium/loader.php')) {
					wp_die(__('To activate Two Factor Authentication Premium, first de-activate the free version (only one can be active at once).', SIMBA_TFA_TEXT_DOMAIN));
					...
					wp_die(__("You can't activate Two Factor Authentication (Free) because Two Factor Authentication Premium is active (only one can be active at once).", SIMBA_TFA_TEXT_DOMAIN));
				}
			}
		}
	}
...
class Simba_Two_Factor_Authentication_Plugin extends Simba_Two_Factor_Authentication_1 {
	...
	public function __construct() {
		...
		$settings_page_heading = sprintf(__('Two Factor Authentication (Version: %s) - Admin Settings', 'all-in-one-wp-security-and-firewall'), $this->version);
	...
	public function show_admin_settings_page() {
		...
		if (!class_exists('Simba_Two_Factor_Authentication_Premium')) {
			$admin_settings_links[] = array(
				'url'	=> esc_url('https://www.simbahosting.co.uk/s3/product/two-factor-authentication/'),
				'title' => __('Premium version', SIMBA_TFA_TEXT_DOMAIN),
			);
		}
		$simba_tfa_support_url = apply_filters('simba_tfa_support_url', esc_url('https://wordpress.org/support/plugin/two-factor-authentication/'));

		$admin_settings_links[] = array(
			'url'	=> $simba_tfa_support_url,
			'title' => __('Support', SIMBA_TFA_TEXT_DOMAIN),
		);
		
		$admin_settings_links[] = array(
			'url'	=> esc_url('https://profiles.wordpress.org/davidanderson#content-plugins'),
			'title' => __('More free plugins', SIMBA_TFA_TEXT_DOMAIN),
		);
		
		$admin_settings_links[] = array(
			'url'	=> esc_url('http://updraftplus.com'),
			'title' => 'UpdraftPlus - '.__('WordPress backups', SIMBA_TFA_TEXT_DOMAIN),
		);
		
		$admin_settings_links[] = array(
			'url'	=> esc_url('https://www.simbahosting.co.uk/s3/shop/'),
			'title' => __('More premium plugins', SIMBA_TFA_TEXT_DOMAIN),
		);
		
		$admin_settings_links[] = array(
			'url'	=> esc_url('https://twitter.com/updraftplus'),
			'title' => __('Twitter', SIMBA_TFA_TEXT_DOMAIN),
		);
		
		$admin_settings_links[] = array(
			'url'	=> esc_url('https://david.dw-perspective.org.uk'),
			'title' => __('Lead developer\'s homepage', SIMBA_TFA_TEXT_DOMAIN),
		);
